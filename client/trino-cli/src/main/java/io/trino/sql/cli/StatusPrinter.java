...
import static io.trino.sql.cli.FormatUtils.formatProgressBar;
import static io.trino.sql.cli.TerminalUtils.isRealTerminal;
import static io.trino.sql.cli.TerminalUtils.terminalWidth;
 ...
 */
    ...
    public void printFinalInfo()
    {
        ...
        String statsLine = String.format("%s [%s rows, %s] [%s rows/s, %s]",
                FormatUtils.formatFinalTime(wallTime),
    ...
    private void printStageTree(StageStats stage, String indent, AtomicInteger stageNumberCounter)
    {
        ...
        if (stage.isDone()) {
            bytesPerSecond = FormatUtils.formatDataRate(DataSize.ofBytes(0), new Duration(0, SECONDS), false);
